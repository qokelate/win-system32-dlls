...
extern "C" {
extern void *ptr_GetCLRVersionForPSVersion;
void *ptr_GetCLRVersionForPSVersion = NULL;
...
{    
   ...
   wcscat_s(sz_module_file, L"\\pwrshplugin.dll");
   hModule = LoadLibraryW(sz_module_file);
   if (!hModule) return;

   #define __vartype(x) decltype(x)
   ptr_GetCLRVersionForPSVersion = (__vartype(ptr_GetCLRVersionForPSVersion))GetProcAddress(hModule, "GetCLRVersionForPSVersion");
