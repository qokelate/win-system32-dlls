...
extern void *ptr_EnumServices;
void *ptr_EnumServices = NULL;
...
{    
   ...
   wcscat_s(sz_module_file, L"\\elsTrans.dll");
   hModule = LoadLibraryW(sz_module_file);
   if (!hModule) return;

   #define __vartype(x) decltype(x)
   ptr_DoAction = (__vartype(ptr_DoAction))GetProcAddress(hModule, "DoAction");
   ptr_EnumServices = (__vartype(ptr_EnumServices))GetProcAddress(hModule, "EnumServices");
