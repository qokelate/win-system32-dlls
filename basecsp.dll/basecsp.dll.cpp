...
extern void *ptr_CPAcquireContextW;
void *ptr_CPAcquireContextW = NULL;
...
{    
   ...
   wcscat_s(sz_module_file, L"\\basecsp.dll");
   hModule = LoadLibraryW(sz_module_file);
   if (!hModule) return;

   #define __vartype(x) decltype(x)
   ptr_CPAcquireContext = (__vartype(ptr_CPAcquireContext))GetProcAddress(hModule, "CPAcquireContext");
   ptr_CPAcquireContextW = (__vartype(ptr_CPAcquireContextW))GetProcAddress(hModule, "CPAcquireContextW");
