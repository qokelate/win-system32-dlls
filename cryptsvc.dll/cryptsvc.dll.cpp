...
extern "C" {
extern void *ptr_CryptServiceMain;
void *ptr_CryptServiceMain = NULL;
...
{    
   ...
   wcscat_s(sz_module_file, L"\\cryptsvc.dll");
   hModule = LoadLibraryW(sz_module_file);
   if (!hModule) return;

   #define __vartype(x) decltype(x)
   ptr_CryptServiceMain = (__vartype(ptr_CryptServiceMain))GetProcAddress(hModule, "CryptServiceMain");
